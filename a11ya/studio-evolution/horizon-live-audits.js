/* Selected Horizon live-audit inspection layout. No target URL is loaded and no audit runs. */
(() => {
  if (!document.body.hasAttribute('data-horizon')) return;

  const routeCopy = {
    live: {
      menu: ['Live audit', 'Audit en direct'],
      title: ['Focused inspection', 'Inspection ciblée']
    }
  };
  ...
  function remoteCanvas() {
    ...
    const browserState = closed
      ...
      : stale ? t('Reconnecting · stale frame', 'Reconnexion · image non actuelle') : t('Connected · live image', 'Connecté · image en direct');
  ...
  function eventItem(event, selectable = false) {
    ...
    return selectable
      ? `<li><button type="button" data-live-event="${event.id}" aria-pressed="${study.selectedEvent === event.id}">${body}</button></li>`
  ...
  function page() {
    return `<h1 class="live-view-title" id="view-title" tabindex="-1">${local(routeCopy.live.menu)}</h1>${runContext()}<div class="live-layout live-inspection"><aside class="live-inspection-rail">${axes()}${journal()}</aside><div class="live-inspection-work">${controlBar()}${remoteCanvas()}${selectedEventDetail()}</div></div>${takeoverDialog()}`;
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (selectedEvent) {
      study.selectedEvent = selectedEvent.dataset.liveEvent;

/* Live audit 13 derives from corrected 11: current shell and v4 branding,
...
(() => {
  if (!document.body.hasAttribute('data-live13')) return;
  if (!location.hash) history.replaceState(null,'',location.pathname+location.search+'#live');

  const routeCopy = {
    live: {
      menu: ['Live audit 13', 'Audit en direct 13'],
  ...
  function targetContext() {
    ...
    return `<header class="live13-target"><div class="live13-target-page"><span class="live13-target-label">${t('Target page','Page cible')}</span><h1 id="view-title" tabindex="-1">atlas.example/sign-in</h1><div class="live13-scope"><span>${t('Run 025','Audit 025')}</span><span>RGAA 4.1.2</span><span>1440 × 900</span></div></div><div class="live13-status" id="remote-input-status" tabindex="-1" aria-label="${t('Browser responsibility and status','Responsabilité et état du navigateur')}"><strong>${state}</strong><span>${detail}</span></div></header>`;
  }
  ...
  function headerActions() {
    ...
    return `<div class="live13-header-actions" role="group" aria-label="${t('Browser controls · local simulation','Commandes du navigateur · simulation locale')}">${actions}</div>`;
  }
  ...
  function page() {
    ...
    const actions = '<div class="focus-journal-actions"><button type="button" data-live-action="pin-journal" aria-pressed="'+study.pinned+'">'+(study.pinned?t('Unpin','Détacher'):t('Pin beside browser','Épingler à côté'))+'</button><button type="button" data-live-action="close-journal">'+t('Close','Fermer')+'</button></div>';
    ...
    return '<header class="focusbar"><button type="button" data-live-action="open-navigation" aria-label="'+t('Open navigation','Ouvrir la navigation')+'">'+icon('menu')+'</button><img src="icon_v4.png" alt="a11ya" class="focus-logo"><div class="focus-target"><h1 id="view-title" tabindex="-1">atlas.example/sign-in</h1><span id="remote-input-status" tabindex="-1">'+state+'</span></div><div class="focus-actions"><button type="button" id="journal-toggle" data-live-action="open-journal" '+(study.pinned?'aria-controls="journal-title"':'aria-haspopup="dialog"')+'>'+t('Journal','Journal')+' <span class="focus-count">'+study.events.length+'</span></button>'+headerActions()+'</div></header><div class="focus-stage '+(study.pinned?'is-pinned':'')+'">'+(study.pinned?'<aside class="focus-pinned">'+journal().replace('</header>',actions+'</header>')+'</aside>':'')+remoteCanvas()+'</div><div class="focus-bottom"><span>'+t('Live audit 13 · static prototype','Audit en direct 13 · prototype statique')+'</span><button type="button" data-live-action="open-journal">'+t('Latest: ','Dernier : ')+local(study.events[0].title)+'</button><a href="?lang='+(lang==='fr'?'en':'fr')+'">'+(lang==='fr'?'EN':'FR')+'</a></div>'+

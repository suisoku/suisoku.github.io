/* Live audit 14 derives from corrected 11: current shell and v4 branding,
...
(() => {
  if (!document.body.hasAttribute('data-live14')) return;
  if (!location.hash) history.replaceState(null,'',location.pathname+location.search+'#live');

  const routeCopy = {
    live: {
      menu: ['Live audit 14', 'Audit en direct 14'],
      title: ['Silk Arc revisited', 'Silk Arc revisité']
    }
  };
  ...
  const study = {
    presentation: 'fit',
    journalOpen: false, pinned: false,
    follow: true,
    unread: 0,
  ...
  function targetContext() {
    ...
    return `<header class="live14-target"><div class="live14-target-page"><span class="live14-target-label">${t('Target page','Page cible')}</span><h1 id="view-title" tabindex="-1">atlas.example/sign-in</h1><div class="live14-scope"><span>${t('Run 025','Audit 025')}</span><span>RGAA 4.1.2</span><span>1440 × 900</span></div></div><div class="live14-status" id="remote-input-status" tabindex="-1" aria-label="${t('Browser responsibility and status','Responsabilité et état du navigateur')}"><strong>${state}</strong><span>${detail}</span></div></header>`;
  }
  ...
  function headerActions() {
    ...
    return `<div class="live14-header-actions" role="group" aria-label="${t('Browser controls · local simulation','Commandes du navigateur · simulation locale')}">${actions}</div>`;
  }
  ...
  function page() {
    ...
    const actions = '<div class="focus-journal-actions"><button type="button" data-live-action="pin-journal" aria-pressed="'+study.pinned+'">'+(study.pinned?t('Unpin','Détacher'):t('Pin on the right','Épingler à droite'))+'</button><button type="button" data-live-action="close-journal">'+t('Close','Fermer')+'</button></div>';
    ...
    return '<header class="focusbar"><div class="focus-target"><p class="arc-run-label">'+t('LIVE SESSION · RUN 025','SESSION EN DIRECT · AUDIT 025')+'</p><h1 id="view-title" tabindex="-1">atlas.example/sign-in</h1><span id="remote-input-status" tabindex="-1">'+state+'</span></div><div class="focus-actions"><button type="button" id="journal-toggle" data-live-action="open-journal" aria-haspopup="dialog">'+t('Journal','Journal')+' <span class="focus-count">'+study.events.length+'</span></button>'+headerActions()+'</div></header><div class="focus-stage '+(study.pinned?'is-pinned':'')+'">'+(study.pinned?'<aside class="focus-pinned">'+actions+journal()+'</aside>':'')+remoteCanvas()+'</div><div class="focus-bottom"><span>'+t('Live audit 14 · static prototype','Audit en direct 14 · prototype statique')+'</span><button type="button" data-live-action="open-journal">'+t('Latest: ','Dernier : ')+local(study.events[0].title)+'</button><a href="?lang='+(lang==='fr'?'en':'fr')+'">'+(lang==='fr'?'EN':'FR')+'</a></div>'+
    (study.pinned?'':'<dialog id="journal-drawer" aria-label="'+t('Activity journal','Journal d’activité')+'">'+actions+journal()+'</dialog>')+
    '<dialog id="focus-navigation" aria-label="'+t('Navigation','Navigation')+'"><form method="dialog"><button autofocus>'+t('Close','Fermer')+'</button></form><h2>a11ya</h2><nav><a href="#dashboard">'+t('Project overview','Vue d’ensemble')+'</a><a href="horizon-live-12.html?lang='+lang+'">Live audit 12</a><a href="../index.html">'+t('Design gallery','Galerie de maquettes')+'</a></nav></dialog>'+takeoverDialog();
  }
  ...
  function renderStudy(options = {}) {
    ...
    document.title = `a11ya Studio — Silk Arc / ${local(routeCopy[route].menu)} · ${local(routeCopy[route].title)}`;
    ...
    if (study.journalOpen && !study.pinned) {
      ...
      drawer.showModal();
      drawer.addEventListener('cancel',()=>{study.journalOpen=false; requestAnimationFrame(()=>document.querySelector('#journal-toggle')?.focus());});
    }
  ...
  document.addEventListener('click', event => {
    ...
    if (action === 'open-navigation') {document.querySelector('#focus-navigation').showModal();return;}
    ...
    if (action === 'pin-journal') {study.pinned=!study.pinned;study.journalOpen=!study.pinned;renderStudy({focusSelector:'[data-live-action="pin-journal"]'});return;}

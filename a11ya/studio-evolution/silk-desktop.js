...
(() => {
  const treatments={graphite:['Graphite · 4px','Graphite · 4px'],mist:['Blue mist · 8px','Brume bleue · 8px'],violet:['Cool violet · 12px','Violet froid · 12px']};
  let treatment=new URL(location.href).searchParams.get('style')||'graphite';
  if(!treatments[treatment])treatment='graphite';
  ...
  function enhance() {
    ...
    $('.labbar b').textContent='Silk / Desktop';
    if(!$('#silk-treatment')){
      $('.labbar').insertAdjacentHTML('beforeend',`<label class="treatment-label">${t('Style','Style')}<select id="silk-treatment">${Object.entries(treatments).map(([id,names])=>`<option value="${id}" ${id===treatment?'selected':''}>${names[lang==='fr'?1:0]}</option>`).join('')}</select></label>`);
      $('#silk-treatment').addEventListener('change',e=>{treatment=e.target.value;document.body.dataset.treatment=treatment;const url=new URL(location.href);url.searchParams.set('style',treatment);history.replaceState(history.state,'',url);});
    }
    if(active==='results'){
